...
export const FinalProgramStep = ({ hash, name, closeModal, onTryAgain, isWaiting, isSuccess, isError }) => {
  const onOkayClick = () => {
    closeModal();
  };
  ...
  const renderContent = useMemo(() => {
      ...
      return {
        ...
        desc: (
          <span className="text-white-500 text-base sm:text-sm">
            Congratulations! <span className="text-white">Program {name}</span> activated
          </span>
        ),
        button: (
    ...
    if (isError) {
      return {
        title: 'Sorry',
        desc: <span className="text-white-500 text-base sm:text-sm">{name} is not activated.</span>,
        button: (
          <>
            <a
  ...
  }, [isWaiting, isError, isSuccess, hash, name]);
